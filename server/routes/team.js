  ...
  Team.findById(({_id: req.params.id}), function (err, item) {
    if (err) {
      return res.status(404).json({
        ...
        item.detailTeam = req.body.detailTeam
...
router.post('/', function (req, res, next) {
  ...
  var team = new Team(req.body)
  team.ownerCompanies = req.user.ownerCompanies
...
//    categories: categoriesArray,
  ...
  if(req.query.search)
    searchQuery['detailTeam.nameTeam'] = new RegExp(req.query.search, 'i')
...
router.delete('/:id', function (req, res, next) {
  Team.findById((req.params.id), function (err, item) {
    if (err) {
      return res.status(500).json({
